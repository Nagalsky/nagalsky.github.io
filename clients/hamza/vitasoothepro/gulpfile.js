...
//Development Tasks
function devHTML(){
  return src(`${options.paths.src.base}/**/*.html`).pipe(rigger()).pipe(dest(options.paths.dist.base));
} 
...
function devScripts(){
  ...
  ]).pipe(rigger()).pipe(concat({ path: 'scripts.js'})).pipe(uglify()).pipe(dest(options.paths.dist.js));
}
