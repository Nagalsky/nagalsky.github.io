...
exports.parse = parse;
exports.defaultTreeAdapter = exports.TokenizerMode = exports.Tokenizer = exports.Parser = exports.ErrorCodes = void 0;
...
})(ERR || (exports.ErrorCodes = ERR = {})); //Const
...
function () {
  ...
  _createClass(Preprocessor, [{
    key: "getError",
    value: function getError(code, cpOffset) {
      var line = this.line,
          col = this.col,
          offset = this.offset;
    ...
    value: function advance() {
      ...
      var isCommonValidRange = this.handler.onParseError === null || cp > 0x1f && cp < 0x7f || cp === CODE_POINTS.LINE_FEED || cp === CODE_POINTS.CARRIAGE_RETURN || cp > 0x9f && cp < 64976;
...
var $ = TAG_ID;
var SPECIAL_ELEMENTS = (_SPECIAL_ELEMENTS = {}, _defineProperty(_SPECIAL_ELEMENTS, NS.HTML, new Set([$.ADDRESS, $.APPLET, $.AREA, $.ARTICLE, $.ASIDE, $.BASE, $.BASEFONT, $.BGSOUND, $.BLOCKQUOTE, $.BODY, $.BR, $.BUTTON, $.CAPTION, $.CENTER, $.COL, $.COLGROUP, $.DD, $.DETAILS, $.DIR, $.DIV, $.DL, $.DT, $.EMBED, $.FIELDSET, $.FIGCAPTION, $.FIGURE, $.FOOTER, $.FORM, $.FRAME, $.FRAMESET, $.H1, $.H2, $.H3, $.H4, $.H5, $.H6, $.HEAD, $.HEADER, $.HGROUP, $.HR, $.HTML, $.IFRAME, $.IMG, $.INPUT, $.LI, $.LINK, $.LISTING, $.MAIN, $.MARQUEE, $.MENU, $.META, $.NAV, $.NOEMBED, $.NOFRAMES, $.NOSCRIPT, $.OBJECT, $.OL, $.P, $.PARAM, $.PLAINTEXT, $.PRE, $.SCRIPT, $.SECTION, $.SELECT, $.SOURCE, $.STYLE, $.SUMMARY, $.TABLE, $.TBODY, $.TD, $.TEMPLATE, $.TEXTAREA, $.TFOOT, $.TH, $.THEAD, $.TITLE, $.TR, $.TRACK, $.UL, $.WBR, $.XMP])), _defineProperty(_SPECIAL_ELEMENTS, NS.MATHML, new Set([$.MI, $.MO, $.MN, $.MS, $.MTEXT, $.ANNOTATION_XML])), _defineProperty(_SPECIAL_ELEMENTS, NS.SVG, new Set([$.TITLE, $.FOREIGN_OBJECT, $.DESC])), _defineProperty(_SPECIAL_ELEMENTS, NS.XLINK, new Set()), _defineProperty(_SPECIAL_ELEMENTS, NS.XML, new Set()), _defineProperty(_SPECIAL_ELEMENTS, NS.XMLNS, new Set()), _SPECIAL_ELEMENTS);
var NUMBERED_HEADERS = new Set([$.H1, $.H2, $.H3, $.H4, $.H5, $.H6]); //States
...
function () {
  function Tokenizer(options, handler) {
    ...
    this.entityDecoder = new EntityDecoder(htmlDecodeTree, function (cp, consumed) {
      // Note: Set `pos` _before_ flushing, as flushing might drop
      // the current chunk and invalidate `entityStartPos`.
      _this.preprocessor.pos = _this.entityStartPos + consumed - 1;

      _this._flushCodePointConsumedAsCharacterReference(cp);
    }, handler.onParseError ? {
      missingSemicolonAfterCharacterReference: function missingSemicolonAfterCharacterReference() {
        _this._err(ERR.missingSemicolonAfterCharacterReference, 1);
      },
      absenceOfDigitsInNumericCharacterReference: function absenceOfDigitsInNumericCharacterReference(consumed) {
        _this._err(ERR.absenceOfDigitsInNumericCharacterReference, _this.entityStartPos - _this.preprocessor.pos + consumed);
      },
      validateNumericCharacterReference: function validateNumericCharacterReference(code) {
        var error = getErrorForNumericCharacterReference(code);
        if (error) _this._err(error, 1);
      }
    } : undefined);
  } //Errors
  ...
  _createClass(Tokenizer, [{
    key: "_err",
    value: function _err(code) {
      var cpOffset = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 0;
    ...
    value: function _emitCodePoint(cp) {
      var type = isWhitespace(cp) ? TokenType.WHITESPACE_CHARACTER : cp === CODE_POINTS.NULL ? TokenType.NULL_CHARACTER : TokenType.CHARACTER;
    ...
    value: function _stateScriptDataDoubleEscapeStart(cp) {
      if (this.preprocessor.startsWith(SEQUENCES.SCRIPT, false) && isScriptDataDoubleEscapeSequenceEnd(this.preprocessor.peek(SEQUENCES.SCRIPT.length))) {
        ...
        for (var i = 0; i < SEQUENCES.SCRIPT.length; i++) {
    ...
    value: function _stateCharacterReference() {
      var length = this.entityDecoder.write(this.preprocessor.html, this.preprocessor.pos);
...
var IMPLICIT_END_TAG_REQUIRED_THOROUGHLY = new Set([].concat(_toConsumableArray(IMPLICIT_END_TAG_REQUIRED), [TAG_ID.CAPTION, TAG_ID.COLGROUP, TAG_ID.TBODY, TAG_ID.TD, TAG_ID.TFOOT, TAG_ID.TH, TAG_ID.THEAD, TAG_ID.TR]));
var SCOPING_ELEMENTS_HTML = new Set([TAG_ID.APPLET, TAG_ID.CAPTION, TAG_ID.HTML, TAG_ID.MARQUEE, TAG_ID.OBJECT, TAG_ID.TABLE, TAG_ID.TD, TAG_ID.TEMPLATE, TAG_ID.TH]);
var SCOPING_ELEMENTS_HTML_LIST = new Set([].concat(_toConsumableArray(SCOPING_ELEMENTS_HTML), [TAG_ID.OL, TAG_ID.UL]));
var SCOPING_ELEMENTS_HTML_BUTTON = new Set([].concat(_toConsumableArray(SCOPING_ELEMENTS_HTML), [TAG_ID.BUTTON]));
...
var TABLE_CELLS = new Set([TAG_ID.TD, TAG_ID.TH]); //Stack of open elements
...
function () {
  ...
  _createClass(OpenElementStack, [{
    ...
    value: function hasInDynamicScope(tagName, htmlScope) {
      for (var i = this.stackTop; i >= 0; i--) {
  ...
  _createClass(FormattingElementList, [{
    ...
    value: function getElementEntryInScopeWithTagName(tagName) {
      ...
      var entry = this.entries.find(function (entry) {
        return entry.type === EntryType.Marker || _this2.treeAdapter.getTagName(entry.element) === tagName;
      });
...
var defaultTreeAdapter = {
  ...
  updateNodeSourceCodeLocation: function updateNodeSourceCodeLocation(node, endLocation) {
    node.sourceCodeLocation = _objectSpread({}, node.sourceCodeLocation, {}, endLocation);
  }
}; //Const
...
function causesExit(startTagToken) {
  ...
  var isFontWithAttrs = tn === TAG_ID.FONT && startTagToken.attrs.some(function (_ref) {
    var name = _ref.name;
...
function () {
  function Parser(options, document) {
    ...
    this.options = _objectSpread({}, defaultParserOptions, {}, options);
  ...
  _createClass(Parser, [{
    ...
    value: function _setDocumentType(token) {
      ...
      if (token.location) {
        ...
        var docTypeNode = documentChildren.find(function (node) {
          return _this3.treeAdapter.isDocumentTypeNode(node);
        });
    ...
    value: function _attachElementToTree(element, location) {
      if (this.options.sourceCodeLocationInfo) {
        var loc = location && _objectSpread({}, location, {
          startTag: location
        });
    ...
    value: function _insertFakeRootElement() {
      var element = this.treeAdapter.createElement(TAG_NAMES.HTML, NS.HTML, []);
    ...
    value: function _reconstructActiveFormattingElements() {
      ...
      if (listLength) {
        var endIndex = this.activeFormattingElements.entries.findIndex(function (entry) {
          return entry.type === EntryType.Marker || _this4.openElements.contains(entry.element);
        });
    ...
    value: function getFragmentParser(fragmentContext, options) {
      var opts = _objectSpread({}, defaultParserOptions, {}, options); //NOTE: use a <template> element as the fragment context if no context element was provided,
...
function doctypeInInitialMode(p, token) {
  ...
  var mode = token.forceQuirks ? DOCUMENT_MODE.QUIRKS : getDocumentMode(token);
...
function aStartTagInBody(p, token) {
  var activeElementEntry = p.activeFormattingElements.getElementEntryInScopeWithTagName(TAG_NAMES.A);
