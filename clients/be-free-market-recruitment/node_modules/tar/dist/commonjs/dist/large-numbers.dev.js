...
var parse = function parse(buf) {
  var pre = buf[0];
  var value = pre === 0x80 ? pos(buf.subarray(1, buf.length)) : pre === 0xff ? twos(buf) : null;
...
var pos = function pos(buf) {
  ...
  for (var i = len - 1; i > -1; i--) {
    var _byte3 = Number(buf[i]);

    if (_byte3 !== 0) {
      sum += _byte3 * Math.pow(256, len - i - 1);
    }
  }
