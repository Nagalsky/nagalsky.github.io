...
gulp.task('html:build', task.html = function () {
	gulp.src(path.src.html) //Выберем наш html
    .pipe(rigger())
	  .pipe(gulp.dest(path.build.html)) //И в build
...
gulp.task('javascript:build', task.javascript = function () {
	gulp.src(path.src.javascript) //Выберем наш JS
	.pipe(gulp.dest(path.build.javascript)) //И в build
    .pipe(browserSync.reload({
      stream: true
    }));
});
...
gulp.task('fonts:build', task.fonts = function () {
    gulp.src(path.src.fonts) //Выберем наш JS
    .pipe(gulp.dest(path.build.fonts)) //И в build
    .pipe(browserSync.reload({
      stream: true
    }));
});
